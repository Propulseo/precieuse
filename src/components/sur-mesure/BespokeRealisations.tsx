...
import { BESPOKE_PIECES, bespokeMessage } from '../../lib/content/bespoke'
import type { BespokePiece } from '../../lib/content/bespoke'

/**
 * Réalisations — 3 duos « à l'atelier / portée », légende centrale (nom +
 * matière) entre deux filets. Données pilotables (props, repli statique).
 */
export function BespokeRealisations({
  pieces = BESPOKE_PIECES,
}: {
  pieces?: BespokePiece[]
}) {
  return (
    <section className="mx-auto max-w-[1180px] px-[8vw] pb-16 pt-[60px]">
      <div className="mb-[54px] text-center">
        <Eyebrow className="mb-[14px] block">{m.sm_real_eyebrow()}</Eyebrow>
      ...
      {pieces.map((piece) => (
        <Reveal key={piece.name}>
          <div className="mb-[26px] grid grid-cols-1 items-stretch gap-5 min-[681px]:grid-cols-2">
            <PieceShot src={piece.atelierSrc} alt={piece.atelierAlt} tag={m.sm_real_tag_atelier()} />
            <PieceShot src={piece.porteeSrc} alt={piece.porteeAlt} tag={m.sm_real_tag_portee()} />
...
function PieceShot({ src, alt, tag }: { src: string; alt: string; tag: string }) {
  return (
