...
import { BESPOKE_VOICES, bespokeMessage } from '../../lib/content/bespoke'
import type { BespokeVoice } from '../../lib/content/bespoke'

/**
 * Témoignages — 3 voix réelles en cartes poudre sur fond poudre-dark, médaillon
 * initiale + attribution (prénom + ville). Données pilotables (props, repli).
 */
export function BespokeVoices({
  voices = BESPOKE_VOICES,
}: {
  voices?: BespokeVoice[]
}) {
  return (
    <section className="bg-poudre-dark px-[8vw] pb-16 pt-[60px]">
      <div className="mb-[54px] text-center">
        <h2 className="font-display text-[clamp(26px,3.3vw,42px)] font-normal leading-[1.1]">
          {m.sm_voices_title()}
        </h2>
      </div>
      <div className="mx-auto grid max-w-[1180px] grid-cols-1 gap-[26px] min-[881px]:grid-cols-3 max-[880px]:max-w-[520px]">
        {voices.map((voice) => (
