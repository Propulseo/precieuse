 ...
 *
 * Perf (NFR LCP mobile < 2,5 s) : chaque vue SSR déclenchait 3 à 9 requêtes
...
const cmsCache = new Map<string, { at: number; ttl: number; data: unknown }>()
...
async function cmsFetch<T>(
  ...
  const hit = cmsCache.get(key)
  if (hit && Date.now() - hit.at < hit.ttl) return hit.data as T | null
  ...
  try {
    ...
    data = await Promise.race([sanity.fetch<T>(query, params ?? {}), timeout])
    clearTimeout(timer)
    if (data === null) {
      console.error('[cms] lecture Sanity trop lente — repli sur le contenu statique')
    }
  } catch (err) {
  ...
  cmsCache.set(key, {
    at: Date.now(),
    ttl: data === null ? NEGATIVE_TTL_MS : CACHE_TTL_MS,
    data,
  })
  return data
}
...
export async function getSite(locale: Locale = DEFAULT_LOCALE): Promise<typeof SITE> {
  ...
  const data = await cmsFetch<Record<string, unknown> | null>(
    `*[_type == "siteSettings"][0]{
      brand, baseline, email, whatsapp, whatsappLabel, instagram, google, address, hours
    }`,
  )
  if (!data) return SITE
  ...
  return {
    ...
    google: String(data.google ?? SITE.google),
    address: {
...
export async function getHomePage(
  ...
  const data = await cmsFetch<Record<string, unknown> | null>(
    `*[_type == "homePage"][0]{
      ...
      bespokeTitle, bespokeIntro, bespokeTagline, bespokeMeta, bespokeCta, bespokeStepLabel,
      ...
      leadCaptureFieldFirstname, leadCaptureFieldLastname, leadCaptureFieldEmail,
      leadCaptureFieldPhone, leadCaptureSubmit, leadCaptureSuccess, leadCaptureError,
      newsletterEyebrow, newsletterTitle, newsletterSubtitle,
      newsletterCta, newsletterPlaceholder,
      seoTitle, seoDescription
    }`,
  )
  if (!data) return fb
  ...
  return {
    hero: {
      ...
      ctaCollection: s(data.heroCtaCollection, fb.hero.ctaCollection),
      ctaBespoke: s(data.heroCtaBespoke, fb.hero.ctaBespoke),
    },
    avantPropos: {
    ...
    sections: {
      ...
      bespoke: {
        ...
        cta: s(data.bespokeCta, fb.sections.bespoke.cta),
        stepLabel: s(data.bespokeStepLabel, fb.sections.bespoke.stepLabel),
      },
      etabli: {
      ...
      leadCapture: {
        ...
        fieldFirstname: s(data.leadCaptureFieldFirstname, fb.sections.leadCapture.fieldFirstname),
        fieldLastname: s(data.leadCaptureFieldLastname, fb.sections.leadCapture.fieldLastname),
        fieldEmail: s(data.leadCaptureFieldEmail, fb.sections.leadCapture.fieldEmail),
        fieldPhone: s(data.leadCaptureFieldPhone, fb.sections.leadCapture.fieldPhone),
        submit: s(data.leadCaptureSubmit, fb.sections.leadCapture.submit),
        success: s(data.leadCaptureSuccess, fb.sections.leadCapture.success),
        error: s(data.leadCaptureError, fb.sections.leadCapture.error),
      },
      newsletter: {
        ...
        cta: s(data.newsletterCta, fb.sections.newsletter.cta),
        placeholder: s(data.newsletterPlaceholder, fb.sections.newsletter.placeholder),
      },
    },
    seo: {
      title: s(data.seoTitle, fb.seo.title),
